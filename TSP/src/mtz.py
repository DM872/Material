...
def solve_MTZ(points, subtours=[]):
    ...
    m.x = po.Var(A, bounds=(0, 1), domain=po.Binary)
    m.u = po.Var(V-{0}, bounds=(0, len(V)-2), domain=po.Reals)

    # Objective
    m.OBJ = po.Objective(expr=sum(tsputil.distance(
        points[e[0]], points[e[1]]) * m.x[e] for e in A), sense=po.minimize)

    # Constraints
    m.mass_balance = po.ConstraintList()
    for v in V:
        m.mass_balance.add(expr=sum(m.x[(v, i)] for i in V if (v, i) in A) == 1)
        m.mass_balance.add(expr=sum(m.x[(i, v)] for i in V if (i, v) in A) == 1)
    ...
    #m.pprint()
    #m.write("mtz.lp")
    solver = po.SolverFactory('gurobi') #glpk
    ...
    if (results.solver.status == po.SolverStatus.ok) and (results.solver.termination_condition == po.TerminationCondition.optimal):
        ...
        print(results)
        return {(i, j): m.x[i, j]() for i, j in A}
    else:
