...
class Data:
    ...
    def is_req_arc(self, edge):
        return (edge[0],edge[1]) in self.A_R or (edge[1],edge[0]) in self.A_R
            ...
            try:
                ...
                for k in range(1,len(elem),4):
                    ...
                    if status == 1: # saltes i valgfri retning
                        ...
                        if not self.is_req_edge( (_from,_to) ) and not self.is_req_arc((_from,_to)):
                            if _from < _to:
                                self.E_R.update( {(_from, _to): {"dem":demand, "len": length}} )
                            else:
                                self.E_R.update( {(_to, _from): {"dem":demand, "len": length}} )
                    ...
                    ## no need _to visit it
                        ...
                        if (_from,_to) not in self.A:
                            self.A.update({(_from, _to): {"dem":demand, "len": length}})
                        if (_to,_from) not in self.A:
                            self.A.update({(_to, _from): {"dem":demand, "len": length}})
